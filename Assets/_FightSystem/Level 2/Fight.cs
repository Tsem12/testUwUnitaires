    ...
    public class Fight
    {
        ...
        public bool IsPriority(Character character1, Character character2)
        {
            if (character1.Speed > character2.Speed)
                return true;
            else if (character1.CurrentEquipment != null && character1.CurrentEquipment.EBonus == EquipementBonus.AttackPriority)
                return true;
            return false;
        }
        ...
        public void PlayAttack(Character attacker, Character defender, bool first)
        {
            ...
            if (canAttack)
                ...
                if (attacker.CurrentStatus != null)
                    attacker.ReceiveAttack(attacker.CurrentStatus.EffectOnAttack(attacker.MaxHealth));
            if (first && defender.IsAlive)
                PlayAttack(defender, attacker, !first);
            if (attacker.CurrentStatus != null) {
                attacker.ReceiveAttack(attacker.CurrentStatus.DamageEndTurn());
